...
class MountainCarAgent:
    def __init__(self, episodes, is_training=True, render=False):
        ...
        if self.is_training:
            ...
            with open('QLearning_mount_car.pkl', 'rb') as f:
    ...
    def run(self):
            ...
            while not terminated and rewards > -1000:  #Reward of -1 for each timestep
                ...
                if self.is_training:
                    self.q[state_p, state_v, action] += self.learning_rate_a * (
                        reward + self.discount_factor_g * np.max(self.q[new_state_p, new_state_v, :]) - self.q[state_p, state_v, action]
                    )
        ...
        if self.is_training:
            with open('QLearning_mount_car.pkl', 'wb') as f:
    ...
    def plot_rewards(self):
        ...
        plt.title('Q-Learning Mountain Car')
        plt.savefig('mountain_car.png')

if __name__ == '__main__':
    # Treinamento
    agent = MountainCarAgent(5000, is_training=True, render=False)
    agent.run()

    # Teste
    agent = MountainCarAgent(1, is_training=False, render=True)
    agent.run()
